...
${PR_CYAN}%${PR_PWDLEN}<...<%~%<<\
${PR_CYAN})$(ruby_prompt_info)${PR_CYAN}${PR_HBAR}${PR_HBAR}${(e)PR_FILLBAR}${PR_HBAR}${PR_CYAN}(\
${PR_CYAN}%(!.%SROOT%s.%n)${PR_CYAN}@${PR_CYAN}%m:%l\
${PR_CYAN})${PR_CYAN}${PR_HBAR}${PR_URCORNER}\

${PR_CYAN}${PR_LLCORNER}${PR_CYAN}${PR_HBAR}(\
${PR_LIGHT_WHITE}%D{%H:%M:%S}\
${PR_LIGHT_WHITE}%{$reset_color%}$(git_prompt_info)$(git_prompt_status)${PR_CYAN})${PR_CYAN}${PR_HBAR}\
${PR_HBAR}\
>${PR_NO_COLOUR} '

# display exitcode on the right when > 0
return_code="%(?..%{$fg[red]%}%? ↵ %{$reset_color%})"
RPROMPT=' $return_code${PR_CYAN}${PR_HBAR}${PR_CYAN}${PR_HBAR}\
(${PR_LIGHT_WHITE}%D{%a,%b%d}${PR_CYAN})${PR_HBAR}${PR_CYAN}${PR_LRCORNER}${PR_NO_COLOUR}'

PS2='${PR_CYAN}${PR_HBAR}\
${PR_CYAN}${PR_HBAR}(\
${PR_LIGHT_WHITE}%_${PR_CYAN})${PR_HBAR}\
